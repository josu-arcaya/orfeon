...
from src.core.utils import Infrastructure, Pipeline, ParetoTools, Objectives, WriteObjectivesToFileObserver
from src.core.utils import StoppingByNonDominance, StoppingByTotalDominance, StoppingByFullPareto
...
class TravelingModel(BinaryProblem):
    ...
    def __init__(self, file_infrastructure, input_pipeline):
    ...
    def create_solution(self) -> BinarySolution:
        new_solution = BinarySolution(number_of_variables=self.number_of_models,
        ...
        for i in range(self.number_of_models):
            new_solution.variables[i] = \
                [True if random.random() > 0.9 else False for _ in range(self.number_of_devices)]
...
class Optimizer():
    ...
    def run(self):
        self.problem = TravelingModel(self.file_infrastructure,
                                      self.input_pipeline)
    ...
    def plot(self):
        # Plot front
        plot_front = Plot(title='Pareto front approximation. Problem: ' + self.problem.get_name(),reference_front=self.problem.reference_front,axis_labels=self.problem.obj_labels)
        plot_front.plot(self.front, label=self.algorithm.label, filename=self.algorithm.get_name())
...
class Constraints():
    ...
    def __privacy(self, location, type):
        ...
        privacy_mask = np.where(self.pipe.privacy_type.to_numpy() == type, 1, 0)[np.newaxis].T
